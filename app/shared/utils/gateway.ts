...
export interface GatewayClient {
  ...
  get: <TResponse>(path: string, req?: Omit<GatewayRequest, 'path' | 'method' | 'body'>) => Promise<TResponse>
  post: <TResponse>(path: string, body?: unknown, req?: Omit<GatewayRequest, 'path' | 'method' | 'body'>) => Promise<TResponse>
}
...
export function createGatewayClient(options: GatewayClientOptions): GatewayClient {
  ...
  async function request<TResponse>(req: GatewayRequest): Promise<TResponse> {
    ...
    try {
      response = await doFetch(buildGatewayUrl(options.baseUrl, options.apiVersion, req.path, req.query), {
        method,
        headers,
        signal: req.signal,
      ...
      throw new GatewayError(
        0,
        { code: 'NETWORK_UNAVAILABLE', message: 'The API Gateway is unreachable.', details: String(cause) },
        correlationId,
      )
    }
    ...
    if (!response.ok) {
      const body: GatewayErrorBody = isErrorBody(payload)
        ? payload
        : { code: 'UNEXPECTED_ERROR', message: `Request ${method} ${req.path} failed (${response.status}).` }
      throw new GatewayError(response.status, body, response.headers.get('X-Correlation-Id') ?? correlationId)
    }
